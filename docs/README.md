...
5.  **`change_order(order, lang, selected_language)`**
    ...
    *   **Возвращает:**
        ...
        *  `"add_item"`: Если пользователь нажал "0. Добавить блюдо"
...
**Глобальные переменные (из `data.py`):**
...
**Переменные внутри функций (описание аналогично предыдущему README):**
(Перечисление переменных внутри функций аналогично предыдущему описанию)
...
*   **`data.py`:** Содержит код для загрузки данных (переменные `LANGUAGES` и `CATEGORIES`) из JSON файлов. Используется для хранения общих переменных и устранения циклических импортов.

## Заключение

Этот скрипт представляет собой функциональный текстовый интерфейс для заказа еды, который имеет хороший потенциал для дальнейшего развития и расширения.